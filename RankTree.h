...
private:
	...
	//Updates the number of nodes in the node's subtree
	void updateN(){
		this->n = this->right->getN()+this->left->getN()+1;
	}
...
public:
    ...
    Node(T info):info(info),parent(0),left(0),right(0),h(0),n(1){}
	...
	//Return amount of nodes in the node's subtree
	int getN(){
		if(this==NULL)
			return 0;
        return n;
    ...
    //Build tree
	RankTree(Compare compare) : compare(compare), head(0),max(0){}
	...
	//Return the i largest element
	bool getIthLargest (int i,T* info){
		if(getN(this->head)<i)
			return false;
		i = getN(this->head) - i + 1; //Depends if you count the Ith from the end or start
		Node* it = this->head;
		while(it){
			if(getN(it->left)+1==i){
				*info = it->info;
